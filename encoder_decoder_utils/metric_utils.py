...
def compute_metrics(
    ...
    if isinstance(tokenizer, tokenizer_utils.DepthTokenizer):
        ...
        reconstruction_accuracy = np.ma.masked_array(
            correct_predictions,
            np.logical_not(target_id_sentinel_tokens),  # Mask out sentence and eos tokens
        ).mean()
        metrics[DepthMetric.SENTENCE_ACCURACY.value] = sentence_accuracy
        metrics[DepthMetric.RECONSTRUCTION_ACCURACY.value] = reconstruction_accuracy
